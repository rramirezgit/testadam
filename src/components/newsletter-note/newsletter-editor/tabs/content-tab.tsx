    ...
    <Box>
      ...
      <Paper sx={{ p: 3 }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
          ...
          <Button variant="outlined" startIcon={<Icon icon="mdi:plus" />} onClick={handleCreateNewNote}>
            Add New Note
          </Button>
        </Box>
        ...
        {selectedNotes.length === 0 ? (
          <Box sx={{ textAlign: "center", py: 4 }}>
            ...
            <Button variant="contained" startIcon={<Icon icon="mdi:plus" />} onClick={handleCreateNewNote}>
              Create New Note
            </Button>
          </Box>
        ) : (
          <Box>
            {selectedNotes.map((note, index) => (
              <Paper key={note.noteId} variant="outlined" sx={{ mb: 2, position: "relative", overflow: "hidden" }}>
                <Box sx={{ p: 2, borderBottom: "1px solid rgba(0,0,0,0.08)", bgcolor: "rgba(0,0,0,0.03)" }}>
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <Box>
                      ...
                      <IconButton size="small" onClick={() => handleMoveNote(note.noteId, "up")} disabled={index === 0}>
                ...
                <Box
                  sx={{
                    ...
                    backgroundImage: note.noteData.selectedBanner ? `url(${note.noteData.selectedBanner})` : "none",
                  ...
                  <Box sx={{ maxHeight: 200, overflow: "hidden", position: "relative" }}>
                    {note.noteData.objdata.slice(0, 3).map((component, compIndex) => {
                      switch (component.type) {
                        ...
                        case "button":
                          return (
                            <Button key={compIndex} variant="contained" size="small" sx={{ mt: 1, mb: 1 }} disabled>
                              {component.content}
                            </Button>
                          )
                        case "divider":
