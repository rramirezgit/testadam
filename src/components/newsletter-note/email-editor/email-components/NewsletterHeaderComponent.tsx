...
const NewsletterHeaderComponent: React.FC<EmailComponentProps> = ({
  component,
  isSelected,
  onSelect,
  onComponentSelect,
  ...
  // Calcular el estilo de fondo
  const backgroundStyle = (() => {
    if (props.useGradient && props.gradientColors && props.gradientColors.length >= 2) {
      return {
        background: `linear-gradient(${props.gradientDirection || 135}deg, ${props.gradientColors[0]}, ${props.gradientColors[1]})`,
      };
    }
    return {
      backgroundColor: props.backgroundColor || '#FFF9CE',
    };
  })();
  ...
  return (
    ...
    >
      <Box sx={{ position: 'relative' }}>
        ...
        <Tooltip title="Header de Newsletter">
          <Box
            sx={{
              position: 'absolute',
              top: -8,
          ...
          >
            <Icon icon="mdi:email-outline" width={10} height={10} />
          </Box>
        </Tooltip>
      </Box>
    </Paper>
...
export default NewsletterHeaderComponent;
